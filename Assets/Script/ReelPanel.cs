...
public class ReelPanel : MonoBehaviour {
	...
	private void InitSymbol(){
		for (int i = 0; i < reelData.playScreen.reels.Length; i++) {
			for (int j = 0; j < reelData.playScreen.reels [i].reel.Length; j++) {
				...
				int[] pos = { i, j };
				reelScreen.getLineSymbol(i,j).GetComponent<SymbolBehavior>().SetMyName(reelData.playScreen.reels[i].reel[0]);
	...
	public void InitImage(){
		for (int i = 0; i < reelData.playScreen.reels.Length; i++) {
			for (int j = 0; j < reelData.playScreen.reels[i].reel.Length; j++) {
				reelScreen.getLineSymbol (i, j).GetComponent<Animator> ().Play (reelData.playScreen.reels[i].reel[j].ToString());
				reelScreen.getLineSymbol (i, j).GetComponent<Animator> ().StartPlayback ();
				//reelScreen.getLineSymbol (i, j).GetComponent<Animator> ().Stop ();
	...
	private IEnumerator playOneByOneAni(){
		...
		for (int j = 0; j < targetsLength; j++) {
			for (int i = 0; i < reelData.cosmos.gameResults.transitions [currentTransition].collapse.targets [j].target.positions.Length; i++) {
				...
				reelScreen.getLineSymbol (p0, reelData.cosmos.gameResults.transitions [currentTransition].collapse.targets [j].target.positions [i].p [1]).GetComponent<Animator> ().StopPlayback ();
			}
